...
internal class ExpressionResolverBuilder : CallSiteVisitor<object, Expression>
{
    internal static readonly MethodInfo InvokeFactoryMethodInfo =
        GetMethodInfo<Action<Func<IServiceProvider, object>, IServiceProvider>>((a, b) => a.Invoke(b));

    internal static readonly MethodInfo CaptureDisposableMethodInfo =
        GetMethodInfo<Func<ServiceProviderEngineScope, object, object>>((a, b) => a.CaptureDisposable(b));

    internal static readonly MethodInfo TryGetValueMethodInfo =
        GetMethodInfo<Func<IDictionary<ServiceCacheKey, object>, ServiceCacheKey, object, bool>>((a, b, c) =>
            a.TryGetValue(b, out c));

    internal static readonly MethodInfo AddMethodInfo =
        GetMethodInfo<Action<IDictionary<ServiceCacheKey, object>, ServiceCacheKey, object>>((a, b, c) => a.Add(b, c));

    internal static readonly MethodInfo MonitorEnterMethodInfo =
        GetMethodInfo<Action<object, bool>>((lockObj, lockTaken) => Monitor.Enter(lockObj, ref lockTaken));

    internal static readonly MethodInfo MonitorExitMethodInfo =
        GetMethodInfo<Action<object>>(lockObj => Monitor.Exit(lockObj));

    internal static readonly MethodInfo ArrayEmptyMethodInfo = typeof(Array).GetMethod(nameof(ArrayEx.Empty));

    private static readonly ParameterExpression ScopeParameter =
        Expression.Parameter(typeof(ServiceProviderEngineScope));
    ...
    private readonly ConcurrentDictionary<ServiceCacheKey, Func<ServiceProviderEngineScope, object>>
        _scopeResolverCache;

    private readonly Func<ServiceCacheKey, ServiceCallSite, Func<ServiceProviderEngineScope, object>>
        _buildTypeDelegate;

    public ExpressionResolverBuilder(CallSiteRuntimeResolver runtimeResolver, IServiceScopeFactory serviceScopeFactory,
    ...
    private Expression<Func<ServiceProviderEngineScope, object>> BuildExpression(ServiceCallSite callSite)
    {
        ...
        return Expression.Lambda<Func<ServiceProviderEngineScope, object>>(VisitCallSite(callSite, null),
            ScopeParameter);
    }
    ...
    protected override Expression VisitServiceScopeFactory(ServiceScopeFactoryCallSite serviceScopeFactoryCallSite,
        object context)
    {
    ...
    protected override Expression VisitIEnumerable(IEnumerableCallSite callSite, object context)
    {
        if (callSite.ServiceCallSites.Length == 0)
        {
            return Expression.Constant(ArrayEmptyMethodInfo
                ...
                .Invoke(obj: null, parameters: ArrayEx.Empty<object>()));
        }
    ...
    protected override Expression VisitConstructor(ConstructorCallSite callSite, object context)
    {
        ...
        {
            parameterExpressions = ArrayEx.Empty<Expression>();
        }
        else
        {
    ...
    private Expression GetCaptureDisposable(ParameterExpression scope)
    {
