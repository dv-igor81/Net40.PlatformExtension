...
using Net40.Microsoft.Extensions.Configuration.Binder;
...
public static class ConfigurationBinder
{
    ...
    {
        var type = typeof(List<>).MakeGenericType(typeInfo.GenericTypeArguments.ToArray()[0]);
    ...
    private static object AttemptBindToCollectionInterfaces(Type type, IConfiguration config, BinderOptions options)
    {
        ...
        if (collectionInterface != null)
        {
            var dictionaryType = typeof(Dictionary<,>).MakeGenericType(typeInfo.GenericTypeArguments.ToArray()[0],
                typeInfo.GenericTypeArguments.ToArray()[1]);
        ...
        if (collectionInterface != null)
        {
            var instance = Activator.CreateInstance(typeof(Dictionary<,>).MakeGenericType(
                typeInfo.GenericTypeArguments.ToArray()[0], typeInfo.GenericTypeArguments.ToArray()[1]));
        ...
        BinderOptions options)
    {
        ...
        var keyType = typeInfo.GenericTypeArguments.ToArray()[0];
        var valueType = typeInfo.GenericTypeArguments.ToArray()[1];
        ...
        BinderOptions options)
    {
        ...
        // ICollection<T> is guaranteed to have exactly one parameter
        var itemType = typeInfo.GenericTypeArguments.ToArray()[0];
    ...
    private static IEnumerable<PropertyInfo> GetAllProperties(TypeInfo type)
    {
        ...
        } while (type.Equals(typeof(object).GetTypeInfo()) == false); //while (type != typeof(object).GetTypeInfo());
